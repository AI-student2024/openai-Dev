...
- [X] 添加对其他语言和翻译方向的支持。
- [X] 添加对保留源 PDF 的原始布局和格式的支持:保留源pdf的图片
- 
- [ ] 添加对多个 PDF 文件的批处理支持。
...
# 从gui界面（Tkinter）启动
python ai_translator/main.py --gui --model_type OpenAIModel --openai_api_key $OPENAI_API_KEY --file_format markdown --book tests/test.pdf --openai_model gpt-3.5-turbo

![PDFTranslator_GUI](images/PDFTranslator_GUI.jpg)


# API调用：
python ai_translator/main.py --api --model_type OpenAIModel --openai_api_key $OPENAI_API_KEY --file_format markdown --book tests/test.pdf --openai_model gpt-3.5-turbo
# 在windows系统cmd终端输入以下post请求，可进行api调用测试：
$ curl -X POST -F "file=@C:\Users\zf\Desktop\mytest.pdf" -F "target_language=zh" http://127.0.0.1:5000/translate_pdf 
请注意：将file=@C:\Users\zf\Desktop\mytest.pdf替换为您实际准备上传的文件路径，将target_language=zh替换为您需要翻译成的目标语言，系统支持的语言为：
supported_languages = {
    'en': 'English',
    'zh': '中文',
    'es': 'Español',
